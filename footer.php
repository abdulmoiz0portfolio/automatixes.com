    ...
    <script type="module">
        ...
        // Sticky button function to open chat or scroll to contact section
        window.connectWithExpert = function(e) {
            if (e && e.preventDefault) e.preventDefault();

            // 1. Try n8n Chat Widget if available
            const toggleContainer = document.querySelector('.chat-window-toggle') || document.querySelector('.n8n-chat button') || document.querySelector('.chat-toggle');
            if (toggleContainer) {
                const stickyBtn = document.getElementById('sticky-expert-btn');
                if (stickyBtn) stickyBtn.style.display = 'none';
                if (typeof toggleChatState === 'function') toggleChatState();
                setTimeout(() => {
                    const textarea = document.querySelector('.chat-layout textarea') || document.querySelector('textarea');
                    if (textarea) {
                        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value").set;
                        if (nativeInputValueSetter) nativeInputValueSetter.call(textarea, "I need to connect with an expert right now.");
            ...
            // 2. Smooth Scroll to Contact Consultation Form
            const contactSec = document.getElementById('contact-section');
            if (contactSec) {
                if (window.lenis) {
                    window.lenis.scrollTo(contactSec, { offset: -60, duration: 1.2 });
                } else {
                    contactSec.scrollIntoView({ behavior: 'smooth' });
                }
                const nameInp = document.getElementById('contact-name') || document.querySelector('#contact-section input');
                if (nameInp) setTimeout(() => nameInp.focus(), 700);
                return;
            }

            // 3. Fallback to Contact Page
            window.location.href = 'contact';
        };

        // Attach direct listener to sticky button
        const sBtn = document.getElementById('sticky-expert-btn');
        if (sBtn) sBtn.addEventListener('click', window.connectWithExpert);
    </script>
